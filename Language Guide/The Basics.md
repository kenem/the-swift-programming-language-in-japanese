...
他の多くの言語とは異なり、Swift ではコード内の文の行末にセミコロン（;）を記述する必要はありませんが、記述しても問題ありません。ただし、1行に複数の文を記述する場合は、文の区切りにセミコロンが必要です。
...
整数は、42や-23などの小数部分がない整数です。 整数は、符号付き（正、0、または負）または符号なし（正または0）のいずれかです。

Swiftには8、16、32、および64ビット形式の符号付きおよび符号なし整数があります。これらの整数は、Cと同様の命名規則に従います。8ビットの符号なし整数は UInt8 型であり、32ビットの符号付き整数は Int32型です。 Swiftのすべての型のように、これらの型の名前は大文字で始まります。

### 最小値および最大値

各整数型の最小値と最大値は、minプロパティとmaxプロパティを使用して参照します。

```
let minValue = UInt8.min  // minValueの値は0, 型は UInt8
let maxValue = UInt8.max  // maxValueの値は255, 型は UInt8
```

minとmaxはそれぞれの数値タイプの型（上記の例のUInt8など）であるため、同じタイプの値とともに式で使用できます。

### Int

通常、特定のサイズの整数を指定する必要はありません。Swiftには、使用するプラットフォームのネイティブワードサイズと同じサイズの整数型Intが存在します。

* 32ビットプラットフォームでは、IntはInt32と同じサイズです。
* 64ビットプラットフォームでは、IntはInt64と同じサイズです。

特定のサイズの整数で作業する必要がない限り、コード内の整数値には常にIntを使用してください。 これにより、コードの一貫性と相互運用性が向上します。32ビットプラットフォームでも、Intは-2,147,483,648から2,147,483,647までの値を格納でき、多くの用途に対して十分な大きさです。

### UInt

Swiftには、使用するプラットフォームのネイティブワードサイズと同じサイズの符号なし整数型UIntも存在します。
...
> ポイント
> 
> プラットフォームのネイティブワードサイズと同じサイズの符号なし整数型が特に必要な場合にのみ、UIntを使用してください。これが当てはまらない場合は、格納する値が負でないことがわかっていても、Intが推奨されます。[型の安全性と型推論]() で説明するように、整数値にIntを一貫して使用すると、コードの相互運用性が向上し、異なる数値型間での変換が不要になり、整数型の推論に一致します。

## 浮動小数点型

浮動小数点数とは、3.14159、0.1、-273.15など、小数点以下を含む数値です。

浮動小数点型は、整数型よりもはるかに広い範囲の値を表すことができ、Intに格納できるよりもはるかに大きい数値や小さい数値を格納できます。 Swiftには2つの符号付き浮動小数点型があります。

* Doubleは64ビットの浮動小数点数を表します。
* Floatは32ビットの浮動小数点数を表します。

> ポイント
> 
> Doubleは小数点以下15桁以上の精度ですが、Floatではわずか6桁の場合があります。どちらのは浮動小数点型を使うかは、処理の性質と必要な数値の範囲によって決めます。どちらを使っても適切に処理ができる状況では、Doubleが推奨されます。

## 型安全性と型推論

Swiftはタイプセーフ（型安全）な言語です。タイプセーフな言語の特性として、使用する型が明確です。例えばString型が想定される場合にIntを渡すことはできません。

Swiftはタイプセーフであるため、コンパイル時にタイプチェックを実施し、不一致の型があればエラーとして報告されます。これにより、開発プロセスのできるだけ早い段階でエラーを捕捉して修正できます。

タイプチェックは、型に纏わるエラーを回避するのに役立ちます。ただし、すべての定数や変数の宣言時に型を指定する必要があるわけではありません。型を指定しなかった場合、Swiftは型推論を使用して適切な型を推測します。型推論により、コンパイラーは、値をもとに、特定の式の型を自動的に推定します。

型推論のため、SwiftはCやObjective-Cなどの言語のように多くの型宣言を必要としません。定数と変数は明示的に型付けされますが、その他の多くのケースでは型宣言は不要です。

型推論は、初期値を使用して定数または変数を宣言する場合に特に便利です。多くの場合、定数または変数の宣言時にリテラル値（直値）を代入したときに型推論が行われます。（リテラル値は、以下の例の42や3.14159など、ソースコードに直接記述される値を指します。）

たとえば、42のリテラル値を、それがどの型であるかを言わずに新しい定数に割り当てると、整数のように見える数値で初期化されているため、Swiftは定数をIntにすることを推論します。

```
meaningOfLife = 42
// meaningOfLifeはInt型と推定されます
```

同様に、浮動小数点リテラルの型を指定しなかった場合、SwiftはDoubleと推測します。

```
pi = 3.14159
// piはDouble型であると推定されます
```

Swiftは、浮動小数点数の型を推論するときに常にFloatではなくDoubleを選択します。

式の中に整数リテラルと浮動小数点リテラルが混在する場合、文脈からDoubleの型が選択されます。

```
let anotherPi = 3 + 0.14159
// anotherPiもDouble型であると推定されます
```

リテラル値3にはそれ自体に明示的な型がないため、加算の一部として浮動小数点リテラルが存在することから、適切な出力型Doubleが選択されます。
